...
class IMU:
    ...
    def actualizar(self, acel, sample_time):
        ...
        #print(aceler)
    	self.state[0] = self.state.x + sample_time*self.state.vx + (((sample_time)**2)/2.0)*(aceler[0]-self.bias_x)
        self.state[1] = self.state.y + sample_time*self.state.vy + (((sample_time)**2)/2.0)*(aceler[1]-self.bias_y)
        self.state[2] = self.state.z + sample_time*self.state.vz + (((sample_time)**2)/2.0)*(aceler[2])
        self.state[3] = self.state.vx + sample_time*(aceler[0])
        self.state[4] = self.state.vy + sample_time*(aceler[1])
        self.state[5] = self.state.vz + sample_time*(aceler[2])
    ...
    def calcular_grad(self, sensor_observation):
        #TODO: Poner aqui como calcular el gradiente
        self.grad = np.dot(1inv(self.P)(self.x_consensus - self.estimated_state))-np.dot(np.dot(self.H.T,inv(self.R)),(self.state-np.dot(self.H,self.state.x)))

    def calcular_hessian(self):
        #TODO: Poner aqui como calcular la hessiana
        self.hessian = nv(self.P) + np.dot(np.dot(self.H.T,inv(self.R)),self.H)

    def calcular_info(self, prices):
        self.PI = self.hessian*np.sum(prices.values())
        return self.PI

    def calcular_x_barra(self, w_k):
        self.estimated_state = np.dot(self.Fk,x_consensus) + w_k
